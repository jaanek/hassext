...
func (db *DB) Insert(r Entity, skip []string) (sql.Result, error) {
	return insert(db, r, skip)
}
...
func DBTags(s interface{}, fieldNames []string) ([]string, string, error) {
	...
	//	if !ok {
	//		return nil, "", fmt.Errorf("DBTags Field name: %s not found in struct!", fieldName)
	//	}
...
func Fields(values interface{}, skip []string) ([]string, string, error) {
	...
	if v.Kind() == reflect.Struct {
		for i := 0; i < v.NumField(); i++ {
			...
			if dbField != "" && !SliceContains(skip, dbField) {
...
func Exec(tx Executor, debug bool, query string, args ...any) (int64, error) {
	if debug {
